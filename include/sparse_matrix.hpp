...
namespace sm
{
    ...
    private:
        ...
        Otor multiply_array( Itor in_first, Itor in_last, Otor out ) const 
        {
            ...
            {
                auto head = std::find_if( (*this).data_.begin(), (*this).data_.end(), [index_first](wrapped_value_type p) { return index_first == isolate_row()(p.first); } );
                auto tail = head;

                for ( auto i = index_first; i != index_last; ++i )
                {
                    head = tail;
                    tail = std::find_if( head, (*this).data_.end(), [i](wrapped_value_type p) { return i+1 == isolate_row()(p.first); } );
        ...
        multiply_array( Itor in_first, Itor in_last, Otor out_first, Otor out_last, size_type index ) const 
        {
            while ( out_first != out_last )
            {
                ...
                auto tail = std::find_if( head, data_.end(), [&index](wrapped_value_type p) { return index++ != isolate_row()(p.first); } );
